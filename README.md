...
`npm run build` will make the **docs** directory ready for drag-n-drop to, for example, https://app.netlify.com/drop (free registration required beforehand).
  ...
  Also, with additionally running `git add docs/styles.css -f` and committing changes, it’s ready for push to GitHub and integration with GitHub Pages. GitHub Pages are free for  public repositories. To enable it, go to your new repository's GitHub page and click the Settings tab (next to Pull Requests, Discussions, and the other tabs). Then enable GitHub Pages by navigating to Pages (from sidebar) → Branch (subsection on the page) and selecting /docs.
